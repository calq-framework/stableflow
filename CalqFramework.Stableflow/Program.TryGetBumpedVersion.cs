...
partial class Program {
    ...
    private bool TryGetBumpedVersion(ICollection<string> projectFiles, string commitHash, out Version result) {
        ...
        if (projectFiles.Any() == false) {
            var latestTagNamex = CMD($"git describe --tags {commitHash}"); // latestTagDescription.Split('/')?[^1]!; // TODO re-validate with regex
        ...
        var latestTagName = CMD($"git describe --tags {commitHash}"); // latestTagDescription.Split('/')?[^1]!; // TODO re-validate with regex
